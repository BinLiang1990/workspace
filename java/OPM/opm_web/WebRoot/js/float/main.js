...
function visNew(id){
	...
	var leftNum = $(window).width/2 + $(document).scrollLeft() - elm.offsetWidth/2;								//针对其它
	var topNum = $(window).height()/20 + $(document).scrollTop() - elm.offsetHeight/2;					//针对其它
	var left = leftNum + 'px';
	var top = topNum + 300 + 'px';
	elm.style.left = left;
	elm.style.top = top;
...
//滚动条控制
function hideScroll(){
	var ST = $(document).scrollTop();	//获取滚动条位置
	var SL = $(document).scrollLeft();	//获取滚动条位置
